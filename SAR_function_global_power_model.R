...
SARmodelGlobal <- function(SAR_database_output, modelrun_name, dataFolder="L:/global_lubio/chapter_2/CHAPTER_II", kOptStrategy="AIC"){
  ...
  parameterization <- function(k, modelingdata){
    ...
    #nb <- dnearneigh(coords, d1=0, d2=k, row.names=modelingdata$id, longlat=TRUE)
    listw <- nb2listw(nb, style="W", zero.policy=TRUE)
    fit <- spautolm(log(srTotal)~log(area),data=modelingdata, listw=listw)  
    result <- c(AIC(fit), 
  ...
  crossvalidation <- function(i, folds){
    ...
    listw <- nb2listw(nb, style="W", zero.policy=TRUE)
    fit <- spautolm(log(srTotal)~log(area), data=train, listw=listw)  
    fit.summary <- summary(fit)
    prediction <- fit.summary$Coef[2]*log(test[,"area"])+fit.summary$Coef[1]
  ...
  listw <- nb2listw(nb, style="W", zero.policy=TRUE)
  fit.final <- spautolm(log(srTotal)~log(area), data=modelingdata, listw=listw)    
